#@-*-coding:utf-8-*-
#@日期时间:2019/8/31 0031 17:38
#@作    者:tanglingqiang
#@Emai    :tanglingqiang@ipin.com
#@File    :read_config.py
...
class ReadExcel(object):
    ...
    def white_excelMax(self, row, value):
        ...
        '''
        self.open()
        max_column = self.sh.max_column  # 获取最大列数
        cell = self.sh.cell(row=row, column=max_column)  # 获取要写入的表格
    ...
    def white_excel(self, row, value, column):
        ...
        '''
        self.open()
        cell = self.sh.cell(row=row, column=column)  # 获取要写入的表格
        cell.value = value  # 写入内容
        self.wb.save(self.file_name)  # 保存数据


if __name__ == '__main__':
    excel_data = ReadExcel(os.path.join(DATA_DIR, 'register.xlsx'), 'sendEmailCode')
    wb = openpyxl.load_workbook(os.path.join(DATA_DIR, 'register.xlsx'))
    sh = wb['sendEmailCode']
    print(sh.max_column)
